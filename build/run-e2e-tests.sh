...
setup_channel_operator() {
	...
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-channel:${APPSUB_COMPONENT_VERSION}|" multicloud-operators-channel/deploy/standalone/operator.yaml
...
setup_subscription_operator() {
	...
	echo "before sed $APPSUB_COMPONENT_VERSION"
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-subscription:$APPSUB_COMPONENT_VERSION|" multicloud-operators-subscription/deploy/standalone/operator.yaml
...
setup_helmrelease_operator() {
	...
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-subscription-release:$APPSUB_COMPONENT_VERSION|" multicloud-operators-subscription-release/deploy/operator.yaml
