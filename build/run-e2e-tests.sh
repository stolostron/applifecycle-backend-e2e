...
setup_channel_operator() {
	...
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-channel:${COMPONENT_VERSION}.0|" multicloud-operators-channel/deploy/standalone/operator.yaml
...
setup_subscription_operator() {
	...
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-subscription:${COMPONENT_VERSION}.0|" multicloud-operators-subscription/deploy/standalone/operator.yaml
...
setup_helmrelease_operator() {
	...
	sed -i -e "s|image: .*$|image: quay.io/open-cluster-management/multicluster-operators-subscription-release:${COMPONENT_VERSION}.0|" multicloud-operators-subscription-release/deploy/operator.yaml
